...
- 支付回调只把订单放入“待第三方开通”，不会从本地账号池自动分配。
- 管理员登录第三方 App 后台创建学生账号，再在本站管理后台点击“录入第三方账号”。账号、密码和到期日以第三方后台实际结果为准。
- 保存后系统尝试通过公众号发送开通通知；未收到时，在“全部订单”点击“补发公众号”。
- 用户在公众号回复“查账号”可查看账号、真实密码和有效期；网页登录信息页要求同一微信授权。
- 第三方服务商如果自行发送短信，本站无法关闭；本站购买链路不调用自有短信服务。
