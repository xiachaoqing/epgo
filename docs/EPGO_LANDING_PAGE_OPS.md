...
1. 用户在 `/epgo/` 微信内授权并完成支付。
2. 支付回调把订单标记为 `vendor_pending`，同时通知管理员。
3. 管理员登录第三方 App 后台创建账号，再在管理后台确认并录入真实账号、密码、到期日。
4. 系统发送公众号开通通知；失败时可在订单列表点击“补发公众号”。用户回复“查账号”或打开登录信息页查看。

第三方 App 没有开放接口，因此第 3 步必须人工完成；本站不调用自有短信服务。
