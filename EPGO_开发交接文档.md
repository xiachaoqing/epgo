...
更新日期：2026-09-20  
当前分支：`main`  
当前提交：`832e90b docs: 增加 EPGO 开发交接文档`
...
- 我的单词本：添加、搜索、标记掌握和删除。
- 7 日 KET 学习计划：按学习方向和每日分钟数生成计划，本地保存。
- 英语抽背卡：读取我的单词本，逐词翻看释义和例句。
...
公众号应作为支付后的服务入口和复购入口。支付可以正常完成，但账号通知必须以用户关注服务号为前提；未关注用户仍可在结果页看到“账号开通中”和“关注入口”，不能把未关注误报成已发送通知。当前 `result.html` 已在 `paid` 但没有账号时显示“账号开通中”，并提供关注状态检查；真正的消息发送、失败记录和补发仍需要后端接口配合。
...
工具不要一次堆几十个，先做家长和英语学习场景：

- 免费引流：学习日历、单词卡、番茄钟、错题备忘录、证件照、图片压缩、图片转 PDF、音频录音。
- 关注后领取：KET 词汇复习表、7 天学习计划、测评解读模板、自然拼读打卡表。
- 低价收费：9.9–19.9 元测评解读或学习计划，29.9–49.9 元 7 天阅读/单词打卡营。
- 高价值服务：老师人工批改、口语录音点评、KET 阶段报告、线下课和 App 套餐。

基础文件工具应尽量浏览器本地处理；需要服务器、AI 或老师人工服务的功能再收费。参考工具站常见的图片、PDF、视频、文本和时间分类，但英语陪跑 GO 的工具页应围绕“孩子英语学习和家长报名决策”，不要做成没有转化目标的通用工具大杂烩。
