...
<nav class="met-head navbar navbar-expand-md" id="mainNav">
    <div class="container">
        <a class="navbar-brand" href="{$c.index_url}" title="{$c.met_webname}">
            ...
            <else/>
                <span class="brand-text">{$c.met_webname}</span>
            </if>
        </a>
    ...
    <div class="qrcode-modal-content">
        ...
        <div style="background:#f7f8fa; padding:10px; border-radius:10px; display:inline-block;">
            <if value="$lang['footinfo_wx']">
                <img src="{$lang.footinfo_wx}" alt="英语陪跑GO公众号" style="width:200px; height:200px; display:block;">
            <else/>
...
.qrcode-modal-content {
    ...
    text-align: center; position: relative; width: 320px;
    box-shadow: 0 20px 60px rgba(0,0,0,0.15); animation: slideUp .3s ease;
}
.qrcode-close {
...
.hamburger-bar { display: block; width: 22px; height: 2px; background: #333; margin: 5px 0; transition: .3s; }
.inner-banner { background: linear-gradient(135deg, #1565C0 0%, #1E88E5 100%); padding: 48px 0 36px; color: #fff; }
